...
function user_search($dbh, $name) {
	if (!$dbh) {
		...
		$result=pg_query($dbh, "SELECT * FROM CMU_User WHERE user_name LIKE '%$the_name%'
...
function get_recommended_posts($dbh, $count = 10, $user) {
	if (!$dbh) {
		...
		(SELECT DISTINCT like_id FROM CMU_Like WHERE like_user_name IN
		...
		GROUP BY post_id, post_user_name, post_title, post_body, post_timestamp, h.like_id
		ORDER BY COUNT(*) DESC
		LIMIT $count;
...
function reset_database($dbh) {
	if (!$dbh) {
		...
		$query = <<<EOD
		DROP TABLE CMU_User CASCADE;
		DROP TABLE CMU_Post CASCADE;
		DROP TABLE CMU_Like CASCADE;
